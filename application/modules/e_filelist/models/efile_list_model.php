...
class Efile_list_model extends CI_Model {
    ...
    public function geteFiles($section_id,$moveup_down,$section_id_search='',$pgmode,$limit,$page)
    {
        ...
        $this->db->select("$tbl_files.file_department_name,$tbl_files.file_Offer_by,$tbl_files.file_id,$tbl_files.file_all_section_no,$tbl_files.file_section_serial_no, $tbl_files.file_number,
$tbl_files.file_uo_or_letter_no, $tbl_files.file_received_emp_id, $tbl_files.file_sender_emp_id,
$tbl_files.file_uo_or_letter_date, $tbl_files.file_type, $tbl_files.file_subject, 
$tbl_files.file_mark_section_id, $tbl_files.file_from_section_id, $tbl_files.file_mark_section_date,
$tbl_files.ps_mark_monitor_date, $tbl_files.ps_moniter_date, $tbl_files.file_judgment_date,
$tbl_files.ps_mark_monitor_date, $tbl_files.createfile_empid, $tbl_files.file_created_date, 
$tbl_files.file_update_date, $tbl_files.file_hardcopy_status, $tbl_files.file_return,
$tbl_files.multi_user_receiver_id, $tbl_files.file_status, $tbl_files.final_draft_id,
$tbl_files.scan_id, $tbl_files.file_subject_en, $tbl_files.file_est_relation_id, 
ft_file_draft.draft_id, ft_file_draft.draft_creater_emp_id, ft_file_draft.draft_subject,
ft_file_draft.draft_content_text, ft_file_draft.draft_type, ft_file_draft.draft_sender_id,
ft_file_draft.draft_reciever_id, ft_file_draft.draft_status, ft_file_draft.notesheet_id,
ft_file_draft.draft_is_finalize, ft_file_draft.draft_update_date, ft_file_draft.draft_create_date, 
ft_file_draft.order_generat_officer_id, 
dept_name_hi, district_name_hi");    
    ...
    {	
		...
		$tbl4 = DEPARTMENTS;
        $tbl5 = DISTRICT;
        $this->db->select($tbl_files .'.*,dept_name_hi,district_name_hi');
		$this->db->join($tbl4, "$tbl4.dept_id = $tbl_files.file_department_id", 'left');
        $this->db->join($tbl5, "$tbl5.district_id = $tbl_files.file_district_id",'left');
		$this->db->where(array('file_id'=>$file_id,'file_hardcopy_status !='=>'close'));
		$query = $this->db->get($tbl_files);
		if($query->num_rows() == 0){
			$this->db->select($tbl_files_master .'.*,dept_name_hi,district_name_hi');
			$this->db->join($tbl4, "$tbl4.dept_id = $tbl_files_master.file_department_id", 'left');
			$this->db->join($tbl5, "$tbl5.district_id = $tbl_files_master.file_district_id",'left');
			$this->db->where(array('file_id'=>$file_id));
			$query = $this->db->get($tbl_files_master);
		}
    ...
    public function sent_efile_close($limit,$page){
		$login_emp_id = emp_session_id();
        $CI = & get_instance();		
		$subQuery = ("SELECT distinct (draft_file_id) FROM `ft_file_draft` WHERE `draft_id` in (SELECT DISTINCT(draft_log_draft_id) FROM `ft_draft_log` where draft_log_creater= ".$login_emp_id . ")");      
		
		$CI->db->select('*')->from(FILES_MASTER)->where("file_id IN ($subQuery) and final_draft_id!='' ", NULL, FALSE);
		if(isset($_GET['searchby']) && $_GET['searchby']!=''){
			$searchval=$_GET['searchby'];
			$sql_emp="SELECT emp_id,emp_full_name FROM ft_employee WHERE MATCH(emp_full_name_hi) AGAINST('".$searchval."')";
			$emp_row_details= get_row($sql_emp);			
			if(isset($emp_row_details['emp_id']) && $emp_row_details['emp_id']!=''){				
				$searchval = $emp_row_details['emp_id'];
				$this->db->where('ft_files_master.file_received_emp_id',$searchval);
			}else{
				$wheres= "(FIND_IN_SET('".$searchval."',ft_files_master.file_all_section_no) OR ft_files_master.file_all_section_no like '%".$searchval."%' OR ft_files_master.file_subject like '%".$searchval."%'  OR ft_files_master.file_uo_or_letter_no like '%".$searchval."%' OR ft_files_master.file_uo_or_letter_date = DATE_FORMAT('".$searchval."', '%Y-%m-%d'))";
				$CI->db->where($wheres);
			}
		}
		$CI->db->order_by("file_update_date", 'desc');
    ...
    public function count_sent_efile_close(){
		
			$login_emp_id = emp_session_id();
        $CI = & get_instance();		
        $subQuery = ("SELECT distinct(draft_file_id) FROM `ft_file_draft` WHERE `draft_id` in (SELECT DISTINCT(draft_log_draft_id) FROM `ft_draft_log_master` where draft_log_creater= ".$login_emp_id ." UNION all SELECT DISTINCT(draft_log_draft_id) FROM `ft_draft_log_master` where draft_log_creater= ".$login_emp_id.")");
		if(isset($_GET['searchby']) && $_GET['searchby']!=''){
			$searchval=$_GET['searchby'];
			$sql_emp="SELECT emp_id,emp_full_name FROM ft_employee WHERE MATCH(emp_full_name_hi) AGAINST('".$searchval."')";
			$emp_row_details= get_row($sql_emp);			
		...
		$CI->db->select('COUNT(DISTINCT(file_id)) as totalfiles')->from(FILES_MASTER)->where("file_id IN ($subQuery) and final_draft_id!='' ", NULL, FALSE);		
		$CI->db->order_by("file_update_date", 'desc');
        $query = $CI->db->get();
        return $query->row_array();
    }
    ...
    public function count_wip(){
        ...
        $this->db->from($tbl1." as a");
        $this->db->join($tbl2." as b"," b.file_id = a.draft_file_id", 'inner');
        $where = "a.draft_reciever_id = '".$login_emp_id."' AND a.draft_sender_id = '".$login_emp_id."' AND (a.draft_status = 2 OR a.draft_status = 3) and b.final_draft_id IS NOT NULL AND b.final_draft_id != '' AND b.file_hardcopy_status != 'close' and a.draft_type='n'";
        $this->db->where($where);
        $query = $this->db->get();
        return $query->row_array();
    ...
    public function count_sent(){
        $login_emp_id = emp_session_id();
        $query3 = "SELECT COUNT(distinct(draft_file_id)) as sent_efile FROM `ft_file_draft` WHERE `draft_id` in
		(SELECT DISTINCT(draft_log_draft_id) FROM `ft_draft_log` where draft_log_creater= ".$login_emp_id." UNION all SELECT DISTINCT(draft_log_draft_id) FROM `ft_draft_log_master` where draft_log_creater= ".$login_emp_id. 
		")";
