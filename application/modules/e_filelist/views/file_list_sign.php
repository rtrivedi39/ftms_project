            ...
            <div class="box" style="overflow: auto" >
					...
					<form enctype='application/json' method="post" id="multi_sign_frm" action="<?php echo base_url(); ?>e_filelist/add_multi_signature" class="no-print">	
                        <table  class="table table-bordered table-striped">
                            <thead>
                            <tr>
                                <th class="no-print"><?php echo $this->lang->line('sno'); ?>
									 </br>
								<b style="font-size: 16px;">Selectall</b> <input type="checkbox" class="" id="selectall" name="" value=""/>
								</th>
                            ...
                            <?php $j=0; $i=1; foreach ($get_files as $key => $files) {?>
                                <tr class="row-gap" alt="<?php echo $j; ?>" id="rowid<?php echo $j; ?>">
                                    <td class="no-print" title="<?php echo $files->file_id;?>">
                                        ...
                                        <br/>
                                        <input type="checkbox" class="slct_file chkbox_m<?php echo $j;?>" id="<?php echo $files->file_id; ?>" name="ck_file_id[<?php echo $j;?>]" value="<?php echo $files->file_id; ?>"/>
                                       </th>
                                        <?php   
									   ...
									   <?php } ?>
										...
										?>	
											...
											<input disabled type="checkbox"  class="file_updown chkbox<?php echo $files->file_id; ?>" value="<?php echo $order_draft_details['draft_id'] ?>" name="file_order_id[]"/>
										 	<b> <?php echo $order_draft_details['notesheet_title']; ?> </b> <a href="<?php echo base_url();?>draft/draft/draft_viewer/<?php echo $order_draft_details['draft_id'] ?>/1" target="_blank"><i class="fa fa-search"></i></a>
											</label>
										<?php 	}
											} ?>
										<?php 	
										...
										?>	<br/><br/>
											...
											<input disabled class="chkbox<?php echo $files->file_id; ?>" type="checkbox" value="<?php echo $other_dept_notsheet_detail[0]['draft_id'] ?>" name="file_other_dpt_nt_id[<?php echo $j;?>]"/>
										 	<b> <?php echo $other_dept_notsheet_detail[0]['notesheet_title']; ?>  <a href="<?php echo base_url();?>draft/draft/draft_viewer/<?php echo $other_dept_notsheet_detail[0]['draft_id'] ?>/1" target="_blank"><i class="fa fa-search"></i></a></b>
											</span>
										<?php } ?>
                                    </td> 
                                    <td><a href="<?php echo base_url()."view_file/viewdetails/".$files->file_id ;?>" target="_blank" data-toggle="tooltip" data-original-title="View details">
				...
				<div class="pull-right" style="margin: 10px;">
					<?php if( ($login_emp_level['emprole_level'] == 6 || $login_emp_level['emprole_level'] <= 7) && $userrole!='25' ){ ?>
                            <button disabled id="sign_button"  type="button" class="btn btn-primary no-print sign_button">हस्ताक्षर करें </button>
						<?php } ?>
				</div>
				<div class="row">
...
<script>
	...
	var emp_level= "<?php echo $login_emp_level['emprole_level']; ?>";
		...
		var checked = $('.chkbox_m'+i).is(':checked');
		var chkboxid = $('.chkbox_m'+i).attr('id');
		
		var total_select_radio_count=$("#total_select_radio_buton_count"+chkboxid).val();
	 if(this.checked){
            $('.chkbox_m'+i).each(function(){
       ...
       if (checked) {
		...
		$("#total_slct_count").val(k);
			...
			$("#total_nu_radio_selected").val(0);
			$(".radiobtn"+chkboxid).prop("checked", false);
			$('.chkbox'+chkboxid).prop("disabled", true); 
			$("#total_slct_count").val(0);
			$(".sign_button").prop("disabled", true);
            });
        }
	}
