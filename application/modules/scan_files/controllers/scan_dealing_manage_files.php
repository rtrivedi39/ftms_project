...
class scan_dealing_manage_files extends MX_Controller {
    ...
    public function manage_files($id = null) {
       // pr($id);
		...
		$add_rk = "";
		 $is_prakaran = $this->input->post('is_prakaran');
        if(!empty($is_prakaran) && $is_prakaran != null && $is_prakaran == '1'){
            $add_rk = "नस्ती/पत्र पर प्रकरण क्रं नही पाया गया  | , ";
        }
        ...
        if ($this->input->post('section_id') == 7 || $this->input->post('section_id') == 25){
    ...
    //        $this->form_validation->set_rules('file_department_id', $this->lang->line('file_department'), 'trim|required|xss_clean');
        ...
        if ($this->form_validation->run($this) === TRUE)
        {
            ...
            $data_dealing = array(
				...
				'file_Offer_by'        		=> $this->input->post('file_offer_by')!=''?$this->input->post('file_offer_by'):'o',
            ...
            $data_dealing_log = array(
                ...
                'flog_other_remark' => $add_rk.'Modify Details by Dealing Assistant',
...
//pr( $data_dealing_log);
         ...
         //   pr($file_other_feilds);
            ...
            if($this->input->post('link_file') == 'link_file'){
               ...
               if($this->input->post('last_url_name')==''){					
					redirect('view_file/dealing_file');
				}else{
					$redir_url = $this->input->post('last_url_name');	
					redirect("$redir_url");
				}
            }
        } else{
