...
class File_search_close_model extends CI_Model {
    ...
    function file_search_sectionno($searchtype = null, $value1 = null , $value2 = null , $year_wise = null,$date_from = null , $to_date = null) {
		...
		$tbl4 = DEPARTMENTS;
        $tbl5 = DISTRICT;
		$tabl7 = FILES_MOVEMENT_MASTER;
        $this->db->select(FILES_MASTER . '.*,'.EMPLOYEES.'.emp_full_name,'.FILES_SECTION.'.section_number,dept_name_hi,district_name_hi,'.$tabl7.'.fmove_created_datetime,'.$tbl6.'.section_number');
        $this->db->from($tbl6);
	...
	function file_search_lists($searchtype = null, $value1 = null , $value2 = null, $bul = '', $floor = '', $year = null) {
		...
		$tbl4 = DEPARTMENTS;
        $tbl5 = DISTRICT;
        $this->db->select(FILES_MASTER . '.*,'.EMPLOYEES.'.emp_full_name,'.FILES_SECTION.'.section_number,dept_name_hi,district_name_hi');
        $this->db->from($tbl1);
		...
		if($year != ''){
			$this->db->where("YEAR(ft_files_master.file_created_date)",$year);
		}
    ...
    function file_search($searchtype = null, $value1 = null , $value2 = null,$limit=null,$page=null,$mark_sections=null ,$year_wise=null,$movement_from_date = null , $movement_to_date = null ) {
        ...
        $tbl5 = DISTRICT;
		 $tabl7 = FILES_MOVEMENT_MASTER;
		$tbl6 = FILES_SECTION;
        $this->db->select(FILES_MASTER . '.*,emp_full_name,dept_name_hi,'.$tbl5.'.district_name_hi,'.$tabl7.'.fmove_created_datetime,'.$tbl6.'.section_number');
		$this->db->from($tbl1);
...
//search foe case no
    function file_search_caseno($searchtype = null, $value1 = null , $value2 = null , $mark_section = null ,$s_files_year= null,$movement_from_date =null, $movement_to_date =null ) {
		...
		$tabl7 = FILES_MOVEMENT_MASTER;
		$tbl8 = FILES_SECTION;
        $this->db->select(FILES_MASTER . '.*,'.EMPLOYEES.'.emp_full_name,'.FILES_OTHER_FEILDS.'.case_no,'.$tabl7.'.fmove_created_datetime,'.$tbl8.'.section_number');
        $this->db->from($tbl1);
        $this->db->join($tbl3, "$tbl3.emp_id = $tbl1.file_received_emp_id", 'left');
    ...
    function file_search_emp_section_wise($searchtype = null, $filter_section_emp_wise = null,$empid_secid = null, $srch_frm_dt = null, $srch_to_dt = null) {
        ...
        $tbl5 = DISTRICT;
        $tbl7 = SECTIONS;
        $this->db->select(FILES_MASTER . '.*,'.EMPLOYEES.'.emp_full_name,'.FILES_OTHER_FEILDS.'.case_no,dept_name_hi,district_name_hi');
        $this->db->from($tbl1);
        $this->db->join($tbl3, "$tbl3.emp_id = $tbl1.file_received_emp_id", 'left');
        $this->db->join($tbl6, "$tbl6.f_file_id = $tbl1.file_id");
        $this->db->join($tbl4, "$tbl4.dept_id = $tbl1.file_department_id", 'left');
	 ...
	 function auto_search_file($searchtype = null, $value1 = null , $value2 = null) {
        ...
        $query2 = $this->db->query("SELECT file_subject,case_parties,case_no,file_id from ft_files_master left join ft_files_other_feilds on ft_files_master.file_id = ft_files_other_feilds.f_file_id where ft_files_master.file_id =".$value1);
        ...
        $tbl3 = EMPLOYEES;
        $tbl6 = FILES_OTHER_FEILDS;
        $this->db->select(FILES_MASTER . '.*,'.EMPLOYEES.'.emp_full_name,'.FILES_OTHER_FEILDS.'.case_no');
        $this->db->from($tbl1);
        $this->db->join($tbl3, "$tbl3.emp_id = $tbl1.file_received_emp_id", 'left');
        $this->db->join($tbl6, "$tbl6.f_file_id = $tbl1.file_id");
