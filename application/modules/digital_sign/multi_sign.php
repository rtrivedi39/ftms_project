...
//print_r($post_data_array);
	foreach($post_data_array as $ky=>$file_data){	
		$file_data['up_down'];
		if($file_data['up_down'] == "leave"){
				$UPDATE_sql = "UPDATE ft_leave_digital_signature SET  ds_affter_signing_content='".trim($file_data['file_param1'])."',
				ds_public_key='".$file_data['public_key']."', ds_signature_key='".$file_data['sign_data']."',
				ds_signature_emp_id=".$file_data['empid'].", ds_emp_name='".$emp_name."' , ds_signature_date='".date('Y-m-d')."',
				ds_is_signature=1,ds_signing_content='".$file_data['signing_content']."', ds_update_date='".date('Y-m-d')."' WHERE ds_leave_mov_id=".$file_data['ck_file_id'];
