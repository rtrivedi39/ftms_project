...
class Manage_service_permission extends MX_Controller {
	...
	public function manage_work_allote($id = ''){
		...
		$data['title'] = $this->lang->line('est_allote_title');
		$data['title_tab'] =  $this->lang->line('est_allote_title_tab');
$data['user_per'] =  $this->service_model->get_permission_with_user();
		if($id == ''){
		...
		if ($this->form_validation->run($this) == TRUE)
		{
            $works = $this->input->post('est_word_alloted_work_id');
            $empid = explode(',',$this->input->post('est_word_alloted_emp_id'));


            foreach($works as $works1){
                $work_allote_form_data = array(
                    'work_master_id' => $works1,
                    'work_user_empid' => $empid[0],
                    'work_user_uniqueid' => $empid[1],
                    'is_processor' => '0',
                );
              $res = insertData($work_allote_form_data, SERVICE_WORK_PREMISSION);
            }
            if($res){
                $this->session->set_flashdata('message', '<div class="alert alert-success alert-dismissable hideauto"><button aria-hidden="true" data-dismiss="alert" class="close" type="button">×</button><i class="icon fa fa-check"></i>'.$this->lang->line('success_message').'</div>');
            }
		   redirect('est_service_module/manage_service_permission/manage_work_allote');
		} else {
        ...
        $data['view_file'] = "manage_service_work";
