   ...
   public function empcate()
    {
		...
		//echo $wr.$bb;die();
		   if($wr == 1 || $bb == 1 || $this->session->userdata('emp_id')  == "1" )
		   {
                $emp_id = $this->uri->segment("3");
                   $emp_month = $this->uri->segment("4");
				   $emp_year = $this->uri->segment("5");
        $data['pay_salary'] = $this->payroll_model->salary_emp($emp_id,$emp_month,$emp_year);
...
$f1 = $this->uri->segment(3);
  ...
  $condi =  array("pay_cate_id"=>$cate_id);
   ...
   //   $data['pay_bills'] = $this->payroll_model->Allbills();
    ...
    {
         $data['title'] = "वेतन जोड़े ";
        $data['title_tab'] = "वेतन जोड़े ";
    $data['msg'] = " "; 
...
if($_POST['condi'] == 1 ){
			...
			redirect("payroll/empcate/".$_POST['cate_id']."/".date("F")."/".date("Y"));
		}else{
		redirect("payroll/showsalary_cate/".$_POST['cate_id']);
		}
    }
    ...
    $data['pay_salary'] = $this->payroll_model->edit_slary_master1();
		if($_POST['condi'] == 1 ){
			...
			redirect("payroll/empcate/".$_POST['pay_salary_cate_id']."/".date("F")."/".date("Y"));
		}else{
  ...
  $data['title'] = $this->lang->line('salary_mastar');
	...
	function addarreasentry()
	{
			...
			if($this->uri->segment(4) == "lpc"){
						$data['title'] = "वेतन  एलपीसी   ";
						
					}else{
						$data['title'] = "वेतन एरियर्स  ";
					}
	...
	function pay_slary_register()
		...
		function cate_emp_list()
		{
			...
			$data['pay_salary'] = $this->payroll_model->cate_emp_list();
			$content = "<select name='emp_order' id='emp_order1'   class='form-control'>
                           <option value=''>कर्मचारी के बाद कर्मचारी की स्थिति को जोड़ने</option>";
						foreach($data['pay_salary'] as $row )
						{
                            $content .="<option value=".$row->emp_order." >".$row->emp_unique_id." ".$row->emp_title_hi."  ".$row->emp_full_name_hi." </option>";
						} 
							 $content .= "</select> ";
