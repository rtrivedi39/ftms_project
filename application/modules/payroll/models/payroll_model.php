...
//echo $this->db->last_query();
	   return $rowid;
	}  public  function getpay1($emp_id,$pay_year)
	{
...
//echo $this->db->last_query();
 ...
 public function salary_emp($cate_id,$m,$y)
    {
        $this->db->select('*');
        $this->db->from('ft_pay_register');
 ...
 $dab = (inputcheckvaul('pay_basic') + inputcheckvaul('pay_grp'));
    ...
    if(inputcheckvaul('cate_id') == 2)
    {
     ...
     $this->db->insert("ft_pay_emp_salary", $datapay);
	  ...
	  $postval = inputcheckvaul('emp_order') +1; 
	 	$dataemp = array( 
		'emp_order' =>  $postval,
		'emp_pay_cate_id'=> inputcheckvaul('cate_id'),
		
		); 
		$this->db->where("emp_unique_id", inputcheckvaul('pay_emp_unique_id'));
        $this->db->update("ft_employee", $dataemp);
		
	 $query1 = $this->db->query("SELECT * FROM `ft_employee` where emp_order > ".inputcheckvaul('emp_order')." and  emp_pay_cate_id =".inputcheckvaul('cate_id'));
        $rowid1 = $query1->result();//sprint_r($rowid1);die();
		 foreach ($rowid1 as $key => $pay) {
				$dataemp1 = array( 
		'emp_order' => $pay ->emp_order +1 ,
		
		);  
		$this->db->where("emp_unique_id", $pay ->emp_unique_id);
        $this->db->update("ft_employee", $dataemp1);
...
function addptremarks()
{
	...
	function checksalaryincement($t)
	{
		$disdeddate = "10-7-".date("Y");
		$date = new DateTime($disdeddate);
		$now = new DateTime();

		if($date < $now) {
		  $curr_financial_year = explode("-",get_financial_year());
		
		 $query = $this->db->query("SELECT * ,ft_pay_increment_month.pay_basic as basic ,ft_pay_increment_month.pay_grp  as grp ,ft_pay_emp_salary.pay_basic as basic1 , ft_pay_emp_salary.pay_grp as grp1 FROM `ft_pay_emp_salary` JOIN ft_employee on ft_employee.emp_unique_id = ft_pay_emp_salary.pay_emp_unique_id join ft_pay_increment_month on ft_employee.emp_unique_id = ft_pay_increment_month.pay_emp_unique_id Join ft_pay_maan_master on ft_pay_maan_master.pay_grade = ft_pay_emp_salary.pay_grp where ft_pay_emp_salary.pay_salary_cate_id =".$t." and ft_pay_increment_month.pay_year=".$curr_financial_year[0]."   ORDER BY `ft_employee`.`emp_order` ASC");
		   $rowid = $query->result();
		   if(sizeof($rowid)== 0)
		   {
			   $this->db->query("INSERT INTO ft_pay_increment_month (pay_emp_unique_id,pay_salary_cate_id,pay_basic, pay_grp,pay_month,pay_year) SELECT pay_emp_unique_id,pay_salary_cate_id,pay_basic, pay_grp ,'July',".$curr_financial_year[0]."  FROM ft_pay_emp_salary");
        ...
        $datapay = array(
			  ...
			  'pay_total_cut' => $this->input->post('pay_total_cut'),
			   ...
			   'pay_remark' => $this->input->post('pay_remark'),
				  'pay_remark' => $this->input->post('pay_remark'),
		       'pay_month_given'=> date("F",strtotime("01-".$this->input->post('pay_month_given')."-".$this->input->post('pay_year_given'))),
						'pay_year_given' =>$this->input->post('pay_year_given'),
						'pay_month_order'=> $this->input->post('pay_month_order'),
        'created_by' => $this->session->userdata('user_id'),
				...
				echo $this->db->last_query();
				 ...
				 if($payid != 0 ){
		  if($this->input->post('computer_bill_number') != ""  &&  $this->input->post('office_bill_number') != "" ){
 ...
 $this->db->select('emp_pay_cate_id');
   ...
   return $rows = $query->result();
	...
	function cate_emp_list(){
		
	  $this->db->select('emp_order,emp_pay_cate_id,emp_unique_id,emp_title_hi,emp_full_name_hi,emp_id');
      $this->db->from('ft_employee');
      $this->db->where("emp_pay_cate_id",$this->uri->segment(3));
	  $this->db->order_by("emp_order","ASC");
      $query = $this->db->get();
     //echo $this->db->last_query();
	return $rows = $query->result();
