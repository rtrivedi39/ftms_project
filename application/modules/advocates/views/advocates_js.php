...
<script type="text/javascript">
	...
	function update_working_status(usr_id,adv_type){
    var usr_id = usr_id;
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            $("#user_id").val(r_data['scm_id']);
            $("#adv_type").val(r_data['advocate_post_type']);
            $("#wk_scm_name_hi_id , #wk_show_name").text(r_data['scm_name_hi']);
            $("#wk_scm_father_name_hi_id").text(r_data['scm_father_name_hi']);
            $("#wk_scm_gender_id").text(sex);
            $("#wk_scm_dob_id").text(r_data['scm_dob']);
            $("#wk_posting_date_id").text(r_data['posting_date']);
            $("#wk_provision_pirod_id").text(r_data['provision_pirod']+' वर्ष');
            $("#wk_post_renew_date_id").text(r_data['post_renew_date']);
            $("#wk_once_registration_number_council_id").text(r_data['once_registration_number_council']);
        }
    });
    $('#update_working_status').attr('action','<?php echo base_url() ;?>advocates/update_working_status');
    $('#modal-update_working_status').modal('show');
}
...
<div class="modal fade" id="modal-update_working_status" data-backdrop="static">
    <div class="modal-dialog">
        <form method="post" id="update_working_status" >
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                <h4 class="modal-title"><i class="fa fa-fw fa-male"></i><span id="wk_show_name"></span> की पद की स्थिति मे बदलाव</h4>
            </div>
                ...
                <div class="box-body table-responsive">
                    <div class="box-body">
                        <div class="row">
                            ...
                            <input type="hidden" name="adv_type" id="adv_type"/>
                            <div class="col-xs-12">
                                <div class="form-group row">
                                    <div class="col-md-5"><label for="exampleInputFile">पूरा नाम </label></div>
                                    <div class="col-md-7"><span id="wk_scm_name_hi_id"></span></div>
                                </div>

                                <div class="form-group row">
                                    <div class="col-md-5"><label for="exampleInputFile">पिता का नाम:</label></div>
                                    <div class="col-md-7"><span id="wk_scm_father_name_hi_id"></span></div>
                                </div>

                                <div class="form-group row">
                                    <div class="col-md-5"><label for="exampleInputFile">प्रथम नियुक्ति दिनांक:</label></div>
                                    <div class="col-md-7"><span id="wk_posting_date_id"></span></div>
                                </div>

                                <div class="form-group row">
                                    <div class="col-md-5"><label for="exampleInputFile">परिवीक्षा काल</label></div>
                                    <div class="col-md-7"><span id="wk_provision_pirod_id"></span></div>
                                </div>

                                <div class="form-group row">
                                    <div class="col-md-5"><label for="exampleInputFile">नवीनीकरण दिनांक:</label></div>
                                    <div class="col-md-7"><span id="wk_post_renew_date_id"></span></div>
                                </div>
                                <div class="form-group row">
                                    <div class="col-md-5"><label> पद की स्थिति :</label></div>
                                    <div class="col-md-7">
                                        <select name="working_status" id="working_status"  class="form-control">
                                            <option value=''> चयन करें </option>
                                            <option value='1'> पद रिक्त </option>
                                            <option value='2'> पद मुक्त </option>
                                            <!--<option value='3'> पद नहीं हैं </option>-->
                                        </select>
                                    </div>
                                </div>

                                <div class="form-group row">
                                    <div class="col-md-5"><label for="exampleInputFile">स्थिति का कारण</label></div>
                                    <div class="col-md-7">
                                        <textarea class="form-control" name="panel_discription" required=""></textarea>
                                    </div>
                                </div>
                                <input type="hidden" name="panel_status" class="rd_btn" value="N"/>
                            </div>
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-danger pull-left" data-dismiss="modal"><i class="fa fa-times"></i> Cancel</button>
                <button id="btn-delete" type="submit"  onclick="return confirm('क्या आप  स्थिति दर्ज करना चहाते है |')"  class="btn btn-primary send_btn"> बदलाव करे</button>
            </div>
        </div>
            </form>
