...
class Advocates extends MX_Controller {
    ...
    public function manage_advocate($adv_type = null ,$id = null) {
		...
		if ($this->form_validation->run($this) == TRUE) {
			...
			if(!empty($_POST['first_appointment_date'])){
			$advocate_master['first_appointment_date'] = date('Y-m-d',strtotime($_POST['first_appointment_date']));
			}
	...
	public function edit_advocate($adv_type = null ,$id = null)
	{
		...
		//pr($advocate_master);
			...
			if($adv_type == 1 || $adv_type == 2  ){
					$dhata_remark = '';
					if( $this->input->post('dhara_twintee_status') == 'y'){
							$dhata_remark = "धारा २० के अंतर्गत कार्यरत हैं| ";
					}
				$advocate_service_record['dhara_twintee_status'] = $this->input->post('dhara_twintee_status');
				$advocate_service_record['dhara_twintee_remark'] = $dhata_remark ;
			}
	...
	public function delete_records_agp_gp($delete_id = null ,$post_type = null )
	{
		//echo ""
		$responce = $this->advocate_model->delete_records_agp_gp($delete_id);
		if($responce){
			$this->session->set_flashdata('message', '<div class="alert alert-success alert-dismissable hideauto"><button aria-hidden="true" data-dismiss="alert" class="close" type="button">×</button><i class="icon fa fa-check"></i>रिकार्ड सफलतापूर्वक डिलीट किया गया | </div>');
			redirect('advocate/list/'.$post_type);
		}
	}	
	public function delete_posting_master($post_id = null ,$post_type_id =null )	{
		$tbl_advocate_post = ADVOCATE_POSTING;	
		$response = delete_data($tbl_advocate_post , array('advocate_posting_id'=>$post_id)) ;
		 if($response){
			 $this->session->set_flashdata('message', '<div class="alert alert-success alert-dismissable hideauto"><button aria-hidden="true" data-dismiss="alert" class="close" type="button">×</button><i class="icon fa fa-check"></i>रिकॉर्ड सफलतापूर्वक डिलीट किया गया है।</div>');
             redirect('advocate/repor_view/'.$post_type_id.'?ac=del');
        }
	}
	
	    public function get_ajax_user_detail()
	{
        $id = $this->input->post('user_id');
        $adv_type = $this->input->post('adv_type');
        $adv_details = $this->advocate_model->getadvocate_details($id ,$adv_type );
        echo json_encode($adv_details[0]);
        exit();

	}
	
	    public function update_working_status()
    {
        $id = $this->input->post('user_id');
        $adv_type = $this->input->post('adv_type');
        if(isset($id) and $id != ''){
            ...
            $res = updateData(ADVOCATE_MASTER, $advocate_master, $condition_1) ;
            if($res){

                $adv_details1 = $adv_details[0];
                $working_text = '';
                if($working_status == '1'){
                    $working_text = 'पद रिक्त';
                }if($working_status == '2'){
                    $working_text = 'पद मुक्त';
                }if($working_status == '3'){
                    $working_text = 'पद नहीं हैं';
                }

                $advocate_master_data = array(
                    'scm_name_en' 		=> $working_text,
                    'scm_name_hi' 		=> $working_text,
                    'scm_post_en' 		=> $adv_details1['scm_post_en'],
                    'scm_post_hi' 		=> $adv_details1['scm_post_hi'],
                    'scm_address_en' 	=> '',
                    'scm_address_hi' 	=> '',
                    'scm_pincode' 		=> '',
                    'advocate_type' 	=> $adv_details1['advocate_type'],
                    'advocate_post_type' => $adv_details1['advocate_post_type'],
                    'advocate_designation' => $adv_details1['advocate_designation'],
                    'scm_state_id' 		=> $adv_details1['scm_state_id'],
                    'scm_district_id' 	=> $adv_details1['scm_district_id'],
                    'scm_tahsil_id' 	=> $adv_details1['scm_tahsil_id'],
                    'scm_taluka_id' 	=> $adv_details1['scm_taluka_id'],
                    'creator_id' 		=> emp_session_id(),
                    'status' 			=> 	1 ,
                    'scm_gender' 		=> '',
                    'scm_dob'	 		=> '',
                    ...
                    'contact_no' 		=> '',
                    'provision_pirod'   =>  '',
                    'once_registration_number_council' =>  '',
                    'notery_registration_no' =>  '',
                    'working_status' =>  $working_status,
                );
                $inserted_id = insertData_with_lastid($advocate_master_data, ADVOCATE_MASTER);
                if($inserted_id){
                    $data1 = array(
                        'advocate_type' 	=> $adv_details1['advocate_type'],
                        'advocate_post_type' => $adv_details1['advocate_post_type'],
                    ...
                    $responce = $this->advocate_model->manage_panel($inserted_id ,$data1);
                    if($responce){
                        $this->session->set_flashdata('message', '<div class="alert alert-success alert-dismissable hideauto"><button aria-hidden="true" data-dismiss="alert" class="close" type="button">×</button><i class="icon fa fa-check"></i>आपके द्वारा '.$adv_details1['scm_name_hi'] .' को '.$working_text.' किया गया है |</div>');
                        redirect($_SERVER['HTTP_REFERER']);
                    }else{
                        redirect($_SERVER['HTTP_REFERER']);
                    }
