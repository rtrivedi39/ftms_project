...
<div class="row" id="eFileTemplate">
								  ...
								  <div class="panel panel-<?php echo @$draft_data['draft_type'] == 'odn' ? 'warning' : 'default'; ?>">
									...
									<div id="collapse_<?php echo @$draft_data['draft_id']; ?>" class="panel-collapse collapse ">
									  ...
									  <?php echo @$draft_data['draft_content_text']; br();
											$verify_status = verify_digital_sinature(@$cor_draft_log[0]->draft_log_id,md5(@$cor_draft_log[0]->draft_content),$file_data['file_hardcopy_status']);
		...
		<div class="box box-success">
						...
						foreach($draft_data as $ky=>$notesheet){
							...
							<div class="mailbox-read-message no-padding" id="for-print">
								<div  style="<?php echo $style; ?>; "  >
									...
									$all_drafts = get_draft_log_data($notesheet->draft_id, true, '5','',$file_data['file_hardcopy_status']);
										...
										<div class="direct-chat-msg">
											...
											<div class="logExpand direct-chat-text" data-toggle="modal" data-target="#expandModel" data-log_id="<?php echo $drafts->draft_log_id ;?>" style="height:120px;overflow-y:auto;background:none;cursor:zoom-in;margin: 5px 0 0 25px;">
												<?php echo filter_string($drafts->draft_content);
													...
													if($type == 'n'){
														$verify_status =  verify_digital_sinature($drafts->draft_log_id,md5($drafts->draft_content),$file_data['file_hardcopy_status']);
														$class_no = '';
			...
			<?php }
					if(check_notesheet_exists($file_data['file_id']) == NULL){					
						if(($file_data['file_return'] != '2') && $file_data['file_received_emp_id'] == $this->session->userdata("emp_id") && ($file_data['file_hardcopy_status'] == 'received' || $file_data['file_hardcopy_status'] == 'working' ) && $emp_role_lvl['emprole_level']==13){
		...
		<div class="col-md-4">
            ...
            <div class="box-footer">
                ...
                <?php } ?>
                   <?php if($file_data['file_return'] != '2' && ($file_data['file_hardcopy_status'] == 'received' || $file_data['file_hardcopy_status'] == 'working' ) && $file_data['file_received_emp_id'] == $emp_session_id) {  ?>
					<button type="button" class="btn btn-success btnaddscandoc" name="btnaddscandoc" data-file_id="<?php echo $file_data['file_id']; ?>"  data-toggle="modal" data-target="#upload_file">दस्तावेज सलग्न करें</button>
				<?php } ?>
            </div><!-- /.box-footer -->
        </div><!-- /. box -->
    </div><!-- /.col -->
...
<div class="modal fade" id="upload_file" tabindex="-1" role="dialog" aria-labelledby="upload_file">
    <div class="modal-dialog">
        <!-- Modal content-->
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal">&times;</button>
                <h4 class="modal-title">Choose and upload</span></h4>
            </div>
            <form action="<?php echo base_url(); ?>draft/upload_document" method="post" enctype="multipart/form-data">
                <div class="modal-body">
					 <div class="form-group">
						<label>दस्तावेज का प्रकार <span class="text-danger">*</span></label>
						<input type="text" name="scan_subfile_types" class="form-control" id="scan_subfile_types"  required>
					 </div>	
					 <div class="form-group">
						<label>दस्तावेज का मेटा  डाटा</label>
						<input type="text" name="save_meta" placeholder="दस्तावेज से सम्बंधित कोई डाटा अगर जरुरी हो तो" class="form-control" id="save_meta">
					 </div>	
					 <div class="form-group">
						  <label for="scapdffile">Upload PDf Scan File</label>
						  <input type="file" name="scapdffile" id="scapdffile" accept=".pdf" required>
						  <input type="hidden" name="scan_files_id" id="scan_files_id">
						  <p class="help-block">एक समय मे एक दस्तावेज अपलोड करें|.</p>
					 </div>	
                </div>
                <div class="modal-footer">
                    <button type="submit" onclick="return confirm('Are you sure to attached new doccument')" class="btn btn-primary">Submit</button>
                    <button type="button" class="btn btn-default pull-left" data-dismiss="modal">Close</button>
                </div>
            </form>
        </div>
...
<script type="text/javascript">
	...
	$(function () {
		...
		$(".btnaddscandoc").click(function () {
			var file_id = $(this).data("file_id");
			$('#scan_files_id').val(file_id);			
		});
