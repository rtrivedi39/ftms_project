                        ...
                        <div class="col-md-12" style="overflow: auto">
                            <table class="table table-bordered table-hover">
                                ...
                                <?php $i=1; foreach ($get_files as $key => $files) {
                                    if($files[0]->file_id != $file_details[0]['file_id']) {?>
                                        <tr>
                                            <td><input type="checkbox" name="linkcheck[]" value="<?php echo $files[0]->file_id;?>"/></td>
                                            <td><?php echo $i;?> (<?php echo $this->lang->line('file_no'); ?> : <?php echo $files[0]->file_id;?>)</td>
                                            <td><?php echo $files[0]->file_subject;?></td>
                                            <td><?php echo $files[0]->file_uo_or_letter_no; ?> (<?php echo getFileType($files[0]->file_type) ;?>)</td>
                                            <td><?php echo date_format(date_create($files[0]->file_uo_or_letter_date), 'd/m/y'); ?></td>
                                            <td><?php echo getSection($files[0]->file_mark_section_id); ?></td>
                                            <td><?php
                                                if(isset($files[0]->section_number)){
                                                    echo $files[0]->section_number;

                                                } else {
                                                    $name_sec = getfilesec_id_byfileid($files[0]->file_id,$files[0]->file_mark_section_id,$files[0]->file_type);
                                                    if(!empty($name_sec)){
                                                        echo $name_sec;
                                                    }else{
                                                        echo 'Na/na';
                                                ...
                                                @$files[0]->section_number ? $files[0]->section_number : getfilesec_id_byfileid($files[0]->file_id,$files[0]->file_mark_section_id,$files[0]->file_type)
                                                ?></td>
                                            <td><?php echo date_format(date_create($files[0]->file_update_date), 'd/m/y'); ?>
                                                (<?php if($files[0]->file_hardcopy_status == 'not'){ echo $this->lang->line('mark_date');} else { echo $this->lang->line('received_date');} ?>)
                                            </td>
                                            <td align=""><?php
                                                $filereceiver = get_user_details($files[0]->file_received_emp_id);
                                                if ($filereceiver)
                                                {
                                                    if($files[0]->file_hardcopy_status == 'not') {
