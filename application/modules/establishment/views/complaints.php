        ...
        <div class="col-xs-12">
            <div class="box box-danger" id="divname">
                <div class="box-header no-print">
					...
					<?php if(check_est_so()) { ?>
                        <div class="btn-group">
                             <a href="?type=p"  class="btn btn-warning <?php echo ($this->input->get('type') && $this->input->get('type') == 'p' || $this->input->get('type') == '') ? 'disabled' : ''; ?>">Pending <span class="badge"><?php echo sizeof($this->complaint_model->get_showcount_complaints('p')) ; ?></span></a>
                            <a href="?type=f"  class="btn btn-success <?php echo ($this->input->get('type') && $this->input->get('type') == 'f') ? 'disabled' : ''; ?>">Finished <span class="badge"><?php echo sizeof($this->complaint_model->get_showcount_complaints('f')) ; ?></span></a>
                            <a href="?type=all"  class="btn btn-primary <?php echo ($this->input->get('type') && $this->input->get('type') == 'all') ? 'disabled' : ''; ?>">All <span class="badge"><?php echo sizeof($this->complaint_model->get_showcount_complaints('all')) ; ?></span></a>
                       </div>
					<?php }  else {
				...
				<?php } ?>
                    ...
                    <table class="table table-bordered table-striped">
                        ...
                        <?php  if((count($get_complaint_list)>0) && is_array($get_complaint_list)) {?>
							...
							foreach (@$get_complaint_list as $key => $complaint) { 
								...
								?>>
									...
									<td class="no-print">
                                        ...
                                        echo   ($complaint['file_hardcopy_status'] == 'received' || $complaint['file_hardcopy_status'] == 'working') && $emp_session_id == $complaint['file_received_emp_id'] ? '<button onclick="open_model6('.$complaint['file_id'].')" class="btn btn-sm btn-twitter btn-block rty6" value="'.$complaint['file_id'].'" data-toggle="tooltip" data-original-title="Mark to DA">शाखा में ही मार्क करे </button>' : false;
                                        echo   ($complaint['file_hardcopy_status'] == 'received' || $complaint['file_hardcopy_status'] == 'working') && $emp_session_id == $complaint['file_received_emp_id'] ? '<button onclick="open_model2('.$complaint['file_id'].')" value="'.$complaint['file_id'].'"  class="btn btn-sm btn-block btn-success upperuser" data-toggle="tooltip" data-original-title="Send to officer">अधिकारी को भेजें</button> ' : false;
                                        ...
                                        echo   ($complaint['file_hardcopy_status'] == 'received' || $complaint['file_hardcopy_status'] == 'working') && $emp_session_id == $complaint['file_received_emp_id'] ? '<button onclick="open_model_dispose('.$complaint['file_id'].')" class="btn btn-sm btn-block btn-danger" value="'.$complaint['file_id'].'" data-toggle="tooltip" data-original-title="Dispose in section">शाखा में Dispose</button> ' : false;
