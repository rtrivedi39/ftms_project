...
class View_file_model extends CI_Model {
    ...
    public function getFiles($section_id,$moveup_down,$section_search_id ='',$limit=null,$page=null)
    {
		...
		if(isset($_GET['searchby']) && $_GET['searchby']!=''){
			$searchval=urldecode(trim($_GET['searchby']));
...
public function count_getFiles($section_id=null,$moveup_down=null,$section_search_id =null)
    {
        $cr_empid = get_emp_by_role('9', '1');
        $cr_explod = explode(',', $cr_empid);
		$empids = $this->get_establishment_empids();
