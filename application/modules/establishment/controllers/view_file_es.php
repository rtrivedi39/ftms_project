    ...
    public function index($id=null)
    {
        ...
        if ($id == 'return') {
		   ...
		   /*Pagination*/			
			...
			if($this->uri->segment(4)== 'return'){
				
				
				if(isset($_GET['section_id'])){
				$config["base_url"] = base_url()."establishment/view_file_es/index/return?section_id=".$_GET['section_id'];
				}
				else{
					$config["base_url"] = base_url()."establishment/view_file_es/index/return?";
				}
			}
			else if($this->uri->segment(4)== 'cr'){
				if(isset($_GET['section_id'])){
				$config["base_url"] = base_url()."establishment/view_file_es/index/cr?section_id=".$_GET['section_id'];
				}
				else{
				$config["base_url"] = base_url()."establishment/view_file_es/index/cr?";
				}
			}
			else{
				$config["base_url"] = base_url()."establishment/view_file_es?";
			}
			
			if(isset($_GET['section_id']) && $_GET['section_id']!=''){
				$config["base_url"] = base_url()."establishment/view_file_es?section_id=".$_GET['section_id'];
			}
						
			$total_count = $this->view_file->count_getFiles($section_id,@$moveup_down,@$_GET['section_id']);								
			...
			if(isset($_GET['per_page']) && $_GET['per_page']!=''){
				...
				if($section_search_id){	
					...
					$data['get_files'] = $this->view_file->getFiles($section_explode, $moveup_down,@$section_search_id,$config['per_page'],$page);
